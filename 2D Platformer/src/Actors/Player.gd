...
func _physics_process(delta: float) -> void:
	# if velocity.y is negative the player is currently jumping
...
func calculate_move_velocity(linear_velocity: Vector2, direction: Vector2, speed: Vector2, is_jump_interrupted: bool) -> Vector2:
	...
	# jump strength based on button released
	if is_jump_interrupted: new_velocity.y = 0.0
	return new_velocity

func calculate_stomp_velocity(linear_velocity: Vector2, impulse: float) -> Vector2:
	var stomp_velocity := linear_velocity
	stomp_velocity.y = -impulse
	return stomp_velocity
