...
func create_map(hypotenuse):
	...
	for y in height:
		...
		for x in length:
			...
			# Add and connect points to astar exluding a border/margin
			if x >= top_margin and x <= (length - bottom_margin):
				...
				# connect to previous point as we move across the hypotenuse
				if x > top_margin:
	...
	for num in range(num_of_astar_obstacles):
		...
		# attempts to space out the obstacles more
